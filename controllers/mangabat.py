...
def comic(request, endpoint):
    ...
    if is404 is not None and "404" in is404.text:
        response = tools.get(f"https://h.mangabat.com/{endpoint}")
        soup = BeautifulSoup(response.text.replace("https://h.mangabat.com/", baseURL), "html.parser")
        is404 = soup.find(style="font: 700 22px sans-serif;")
...
def search(request, query):
    ...
    for page in pagination:
        ...
        if url is None:
            endpoint = None
        elif "h.mangabat.com" in url:
            endpoint = url.replace(altURL, "").replace("/manga", "")
        elif "mangabat.com" in url:
...
def genres(request, type, page):
    ...
    is404 = soup.find(style="font: 700 22px sans-serif;")
    if is404 is not None and "404" in is404.text:
        response = tools.get(f"https://h.mangabat.com/{type}")
        soup = BeautifulSoup(response.text, "html.parser")
        is404 = soup.find(style="font: 700 22px sans-serif;")
